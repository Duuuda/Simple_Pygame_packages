#
# colors.py: a Pygame based colors constants module for Python
# Version 1.0b - 20. 12. 2019
#
...
class Colors:
    ...
    @staticmethod
    def change_the_colour(color, coefficient):
        new_color = list()
        for i in color:
            item = i + coefficient
            if item < 0:
    ...
    def random_color(self):
        lst = self.__dir__()[:self.__dir__().index('white') + 1]
        return self.__getattribute__(lst[randint(0, len(lst) - 1)])

    @staticmethod
    def absolute_random_color():
